...
make mrproper
cp $TARGET_CONFIG_FILE $KERNEL_BUILD_BUILTIN_PATH/.config
...
make -j$(nproc) CC=clang KCFLAGS='-w' vmlinux O=$KERNEL_BUILD_BUILTIN_PATH
...
make CC=clang olddefconfig O=$KERNEL_BUILD_PATH
make -j$(nproc) CC=clang KCFLAGS='-w -fno-inline-functions -fno-inline-small-functions -fno-inline-functions-called-once -U__OPTIMIZE__' vmlinux modules O=$KERNEL_BUILD_PATH
